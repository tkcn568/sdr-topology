...
@main.command()
@click.option("--freq", "-f", required=True, type=int,
              help="Center frequency in Hz (e.g. 99500000)")
@click.option("--rate", "-r", default=250_000, show_default=True, type=int,
              help="Sample rate in Hz")
@click.option("--samples", "-n", default=250_000, show_default=True, type=int,
              help="Number of samples to capture")
@click.option("--gain", "-g", default="auto", show_default=True,
...
def capture(freq, rate, samples, gain, output, notes):
    ...
    console.print(f"[bold]Capturing[/bold] {samples:,} sample at {freq / 1e6:.3f} MHz...")
...
@main.command()
@click.option("--capture-path", "-c", required=True, type=click.Path(exists=True),
              help="Path to .npy capture file")
@click.option("--method", "-m", default="iq", show_default=True,
              type=click.Choice(["iq", "delay"]),
              help="Embedding method")
@click.option("--label", "-l", required=True,
              help="Signal type label for the profile entry")
@click.option("--n-points", default=2000, show_default=True, type=int,
              help="Number of points to embed")
@click.option("--dim", default=None, type=int,
              help="Embedding dimension (delay only; auto-selected if omitted)")
@click.option("--tau", default=None, type=int,
              help="Time delay in samples (delay only; auto-selected if omitted)")
@click.option("--maxdim", default=1, show_default=True, type=int,
              help="Maximum homology dimension for persistence")
@click.option("--library", default=str(DEFAULT_LIBRARY), show_default=True,
              type=click.Path(),
...
def analyze(capture_path, method, label, n_points, dim, tau,
            maxdim, library, notes, no_save):
...
@click.option("--label", default=None,
              help="Compare all entries with this label pairwise")
@click.option("--dim", default=1, show_default=True, type=int,
...
def profile_compare(key_a, key_b, label, dim, library):
    ...
    if key_a and key_b:
        ...
        console.print(
            "[red]Provide either --key-a and --key-b, or --label.[/red]"
        )
        raise click.Abort()
